...
public class LecteurDonnees {
    ...
    public static DonneesSimulation creeDonnees(String fichierDonnees)
        ...
        Incendie incendies[] = lecteur.creeIncendies(carte);
        ...
        try {
            ...
            for (int i = 0; i < nbRobots; i++) {
                try {
                    ...
                    switch (type) {
                        case "DRONE": {
                            robot = new Drone(s, carte.getCase(lig, col), carte);
                            break;
                        }
                        case "PATTES": {
                            robot = new Pattes(s, carte.getCase(lig, col), carte);
                            break;
                        }
                        case "CHENILLES": {
                            robot = new Chenilles(s, carte.getCase(lig, col), carte);
                            break;
                        }
                        case "ROUES": {
                            robot = new Roues(s, carte.getCase(lig, col), carte);
                            break;
                        }
                        default:
        ...
        try {
            ...
            for (int i = 0; i < nbIncendies; i++) {
                ...
                try {
                    ...
                    verifieLigneTerminee();
                    incendies[i] = new Incendie(carte.getCase(lig, col), intensite);
                } catch (NoSuchElementException e) {
                    throw new DataFormatException("format d'incendie invalide. "
                            + "Attendu: ligne colonne intensite");
                }
    ...
    private Carte creeCarte() throws DataFormatException {
        ...
        try {
            ...
            ArrayList<Case> Eau = new ArrayList<Case>();
            for (int lig = 0; lig < nbLignes; lig++) {
                for (int col = 0; col < nbColonnes; col++) {
                    ignorerCommentaires();
                    String chaineNature = new String();
                    try {
                        ...
                        verifieLigneTerminee();
                        Case casse = new Case(lig, col, nature);
                        cases[lig][col] = casse;
                        if (casse.getNature() == NatureTerrain.EAU) {
                            Eau.add(casse);
                        }
            ...
            Carte carte = new Carte(nbLignes, nbColonnes, tailleCases, cases, Eau);
            return carte;
        } catch (NoSuchElementException e) {
            throw new DataFormatException("Format invalide. "
                    + "Attendu: nbLignes nbColonnes tailleCases");
